...
const SessionJoinPage = ({DOMAIN}) => {
    ...
    useEffect(() => {
        const socket = new WebSocket(`${DOMAIN.split('://')[0] === 'http' ? 'ws' : 'wss'}://${DOMAIN.split('://')[1]}`);
    ...
    <div>
        ...
        <h2 className='session-title'>{sessionJoinerValue || 'Awaiting join Id'}</h2>
        <p className='session-text'>Or type in the session id</p>
        <input className='primary-input' placeholder='Session Id' type='text' onChange={(e) => setSessionId(e.target.value)} />
        <div className='centerize' style={{marginBottom: 50}}>
            <button className='primary-button' onClick={GoToSession}>Go To Session</button>
        </div>
